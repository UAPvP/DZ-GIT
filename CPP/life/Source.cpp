...
public:
    ...
    void Tick()
    {
        const size_t actors_count = actors.size();
...
public:
    ...
    bool IsFull() const { return hunger >= max_hunger; }
...
public:
    ...
    void Tick() override
    {
        ...
        if (IsFull())
        {
            GetScene()->AddActor(new Rabbit(GetScene(), max_hunger, max_hunger));
        }
    }
};
...
{
public:
    Fox(Scene* scene, const int hunger, const int max_hunger) : Animal(scene, hunger, max_hunger) {}
    ...
    void Tick() override
    {
        ...
        if (IsFull())
        {
            GetScene()->AddActor(new Fox(GetScene(), max_hunger, max_hunger));
        }
    }
};
...
int main()
{
    ...
    for (size_t i = 0; i < 5; i++)
    {
        const int max_hunger = std::rand() % 6 + 5;
